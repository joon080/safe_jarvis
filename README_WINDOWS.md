...
```powershell
# 전체 파이프라인
python actions/dual_agent_pipeline.py --task "로그인 폼 유효성 검사 버그 고쳐줘" --project "C:\Users\08seo\projects\AAU-niversity"

# 작업 지시를 파일로
python actions/dual_agent_pipeline.py --task-file task.txt --project <대상경로>

# 토큰 소비 없이 명령/프롬프트만 확인 (먼저 이걸로 점검 추천)
python actions/dual_agent_pipeline.py --task "테스트" --project <대상경로> --dry-run

# 단일 단계만
python actions/dual_agent_pipeline.py --only claude_plan --project <대상경로>

# 중간 단계부터 재개
python actions/dual_agent_pipeline.py --from-step codex_code_review --project <대상경로>

# 이전 산출물 초기화 후 재실행 (task.md 는 유지)
python actions/dual_agent_pipeline.py --task "..." --project <대상경로> --reset
```
